...
export interface MockupSet {
  ...
  project: string;
  ticket?: string;
  // Lineage: parent is the slug of the group this one was varied from (empty
  // for a root group); parentFile is the option within parent that was varied;
  // instructions is the free-text change request that produced this group.
  parent?: string;
  parentFile?: string;
  instructions?: string;
...
export interface MockupBindings {
...
let bindings: (() => MockupBindings) | null = null;
let sets: MockupSet[] = [];
let active: MockupSet | null = null;
let activeOption = 0;
...
let pendingVariationParent: string | null = null;
...
export function setPendingMockupSlug(slug: string): void {
  pendingSlug = slug;
}
...
export function setChosenMockup(slug: string, file: string): void {
  chosenSlug = slug;
  chosenFile = file;
}
...
function childrenOf(slug: string): MockupSet[] {
  return sets
...
function setBySlug(slug: string): MockupSet | null {
...
// never breaks.
function pathToRoot(set: MockupSet): MockupSet[] {
  const chain: MockupSet[] = [];
  const seen = new Set<string>();
  let cur: MockupSet | null = set;
...
function rootFor(set: MockupSet): MockupSet {
  ...
  h.innerHTML =
    ...
    roots
      .map((s) => {
        ...
        return (
          `<button class="mockup-card" data-i="${i}" type="button">` +
      ...
      .join("") +
    `</div>`;
  h.querySelectorAll<HTMLButtonElement>(".mockup-card").forEach((card) => {
...
function crumbLabel(set: MockupSet): string {
  if (!set.parent) return set.feature;
...
function renderDetail(): void {
  ...
  const winnerPath = winner ? new Set(pathToRoot(winner).map((s) => s.slug)) : new Set<string>();
  ...
  const crumbs = pathToRoot(set)
    .map((s, idx, arr) => {
      ...
      return (
        `<button class="${cls}" data-slug="${escapeHtml(s.slug)}" type="button"${last ? " disabled" : ""}>` +
  ...
  const tabs = set.options
    .map((o, i) => {
      ...
      const mark = chosen ? "✓ " : "";
      return (
        `<button class="${cls}" data-i="${i}" title="${escapeHtml(o.description)}" type="button">` +
  ...
  const kidChips = kids
    .map((c) => {
      ...
      return (
        `<button class="mockup-child-chip${onPath ? " on-winner-path" : ""}" data-slug="${escapeHtml(c.slug)}" type="button">` +
  ...
  h.querySelectorAll<HTMLButtonElement>(".mockup-tab").forEach((tab) => {
  ...
  h.querySelectorAll<HTMLButtonElement>(".mockup-crumb").forEach((crumb) => {
  ...
  h.querySelectorAll<HTMLButtonElement>(".mockup-child-chip[data-slug]").forEach((chip) => {
  ...
  h.querySelectorAll<HTMLButtonElement>(".mockup-ctl").forEach((ctl) => {
...
async function handleControl(
  ...
  switch (act) {
    ...
    case "vary-cancel":
      ...
      break;
    case "vary-submit": {
      const input = h.querySelector<HTMLTextAreaElement>(".mockup-vary-input");
    ...
    case "unchoose":
      ...
      break;
    case "prune": {
      if (!window.confirm("Prune this variation branch? It is archived and removed from navigation.")) return;
...
export async function showMockups(): Promise<void> {
  const h = host();
...
export function initMockupsView(getBindings: () => MockupBindings): void {
